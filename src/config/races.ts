import { DivisionsType, DivisionType, LevelsType, RaceType } from "@/types";

export const genders = {
  girls: "Girls",
  boys: "Boys",
};

export const divisions: DivisionsType = {
  one: { num: 1, numRoman: "I", name: "Blue" },
  two: { num: 2, numRoman: "II", name: "Gold" },
  three: { num: 3, numRoman: "III", name: "Red" },
  four: { num: 4, numRoman: "IV", name: "White" },
  five: { num: 5, numRoman: "V", name: "Green" },
};

export const scheduleLevelGenderToken = "%%gender%%";
export const scheduleLevelSweepstakesNameToken = "%%sweepstakesName%%";

export const levels: LevelsType = {
  novice: {
    level: "Novice",
    isIndivOnly: true,
    resultLink: "n",
    resultOrder: 1,
  },
  frosh: {
    level: "Frosh",
    resultLink: "f",
    resultOrder: 2,
  },
  soph: {
    level: "Soph",
    resultLink: "s",
    resultOrder: 3,
  },
  juniorVarsity: {
    level: "Junior Varsity",
    resultSpan2: true,
    resultLink: "jv",
    resultOrder: 4,
  },
  juniorSenior: {
    level: "Junior / Senior",
    resultSpan2: true,
    resultLink: "js",
    resultOrder: 4,
  },
  junior: {
    level: "Junior",
    resultLink: "j",
    resultOrder: 4,
  },
  senior: {
    level: "Senior",
    resultLink: "sen",
    resultOrder: 5,
  },
  varsity: {
    level: "Varsity",
    resultLink: "v",
    resultOrder: 6,
  },
  rated: {
    level: "Rated",
    resultLink: "rated",
    resultOrder: 8,
  },
  sweepstakes: {
    level: "Sweepstakes",
    scheduleFormat: `${scheduleLevelSweepstakesNameToken} ${scheduleLevelGenderToken} Sweepstakes`,
    resultLink: "sweep",
    resultOrder: 7,
  },
};

export const sweepstakesLevelName = {
  [genders.girls]: "Bob Day's",
  [genders.boys]: "Doug Speck's",
};

export const heats = {
  a: "A",
  b: "B",
};

export const races2007to2012: RaceType[] = [
  {
    num: 1,
    time: "5:00 PM",
    divisions: [divisions.three, divisions.four],
    level: levels.novice,
    gender: genders.girls,
  },
  {
    num: 2,
    time: "5:20 PM",
    divisions: [divisions.three, divisions.four],
    level: levels.novice,
    gender: genders.boys,
  },
  {
    num: 3,
    time: "5:40 PM",
    divisions: [divisions.four],
    level: levels.juniorVarsity,
    gender: genders.girls,
  },
  {
    num: 4,
    time: "5:55 PM",
    divisions: [divisions.three],
    level: levels.juniorVarsity,
    gender: genders.girls,
  },
  {
    num: 5,
    time: "6:10 PM",
    divisions: [divisions.four],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 6,
    time: "6:25 PM",
    divisions: [divisions.three],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 7,
    time: "6:40 PM",
    divisions: [divisions.four],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 8,
    time: "6:55 PM",
    divisions: [divisions.three],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 9,
    time: "7:15 PM",
    divisions: [divisions.four],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 10,
    time: "7:30 PM",
    divisions: [divisions.three],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 11,
    time: "7:45 PM",
    divisions: [divisions.four],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 12,
    time: "8:00 PM",
    divisions: [divisions.three],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 13,
    time: "8:15 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    gender: genders.girls,
  },
  {
    num: 14,
    time: "8:30 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    gender: genders.girls,
  },
  {
    num: 15,
    time: "8:50 PM",
    divisions: [divisions.four],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 16,
    time: "9:05 PM",
    divisions: [divisions.three],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 17,
    time: "9:20 PM",
    divisions: [divisions.four],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 18,
    time: "9:35 PM",
    divisions: [divisions.three],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 19,
    time: "3:45 PM",
    divisions: [divisions.one, divisions.two],
    level: levels.novice,
    gender: genders.girls,
  },
  {
    num: 20,
    time: "4:10 PM",
    divisions: [divisions.one, divisions.two],
    level: levels.novice,
    gender: genders.boys,
  },
  {
    num: 21,
    time: "4:30 PM",
    divisions: [divisions.two],
    level: levels.juniorVarsity,
    gender: genders.girls,
  },
  {
    num: 22,
    time: "4:45 PM",
    divisions: [divisions.one],
    level: levels.juniorVarsity,
    gender: genders.girls,
  },
  {
    num: 23,
    time: "5:00 PM",
    divisions: [divisions.two],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 24,
    time: "5:15 PM",
    divisions: [divisions.one],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 25,
    time: "5:30 PM",
    divisions: [divisions.two],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 26,
    time: "5:45 PM",
    divisions: [divisions.one],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 27,
    time: "6:05 PM",
    divisions: [divisions.two],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 28,
    time: "6:20 PM",
    divisions: [divisions.one],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 29,
    time: "6:35 PM",
    divisions: [divisions.two],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 30,
    time: "6:50 PM",
    divisions: [divisions.one],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 31,
    time: "7:05 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    gender: genders.girls,
  },
  {
    num: 32,
    time: "7:20 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    gender: genders.girls,
  },
  {
    num: 33,
    time: "7:40 PM",
    divisions: [divisions.two],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 34,
    time: "7:55 PM",
    divisions: [divisions.one],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 35,
    time: "8:10 PM",
    divisions: [divisions.two],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 36,
    time: "8:25 PM",
    divisions: [divisions.one],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 37,
    time: "8:40 PM",
    level: levels.rated,
    gender: genders.girls,
  },
  {
    num: 38,
    time: "9:00 PM",
    level: levels.rated,
    gender: genders.boys,
  },
  {
    num: 39,
    time: "9:20 PM",
    level: levels.sweepstakes,
    gender: genders.girls,
  },
  {
    num: 40,
    time: "9:40 PM",
    level: levels.sweepstakes,
    gender: genders.boys,
  },
];

export const races2013to2022: RaceType[] = [
  {
    num: 1,
    time: "5:00 PM",
    divisions: [divisions.three, divisions.four],
    level: levels.novice,
    gender: genders.boys,
  },
  {
    num: 2,
    time: "5:20 PM",
    divisions: [divisions.three, divisions.four],
    level: levels.novice,
    gender: genders.girls,
  },
  {
    num: 3,
    time: "5:40 PM",
    divisions: [divisions.four],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 4,
    time: "5:52 PM",
    divisions: [divisions.three],
    level: levels.juniorVarsity,
    gender: genders.girls,
  },
  {
    num: 5,
    time: "6:04 PM",
    divisions: [divisions.four],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 6,
    time: "6:16 PM",
    divisions: [divisions.three],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 7,
    time: "6:28 PM",
    divisions: [divisions.four],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 8,
    time: "6:40 PM",
    divisions: [divisions.three],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 9,
    time: "6:52 PM",
    divisions: [divisions.four],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 10,
    time: "7:04 PM",
    divisions: [divisions.three],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 11,
    time: "7:16 PM",
    divisions: [divisions.four],
    level: levels.juniorVarsity,
    gender: genders.girls,
  },
  {
    num: 12,
    time: "7:28 PM",
    divisions: [divisions.three],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 13,
    time: "7:40 PM",
    divisions: [divisions.four],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 14,
    time: "7:52 PM",
    divisions: [divisions.three],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 15,
    time: "8:04 PM",
    divisions: [divisions.four],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 16,
    time: "8:16 PM",
    divisions: [divisions.three],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 17,
    time: "8:28 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 18,
    time: "8:40 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 19,
    time: "8:52 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 20,
    time: "9:04 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 21,
    time: "9:16 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 22,
    time: "9:28 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 23,
    time: "9:40 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 24,
    time: "9:52 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 25,
    time: "3:50 PM",
    divisions: [divisions.one, divisions.two],
    level: levels.novice,
    gender: genders.boys,
  },
  {
    num: 26,
    time: "4:10 PM",
    divisions: [divisions.one, divisions.two],
    level: levels.novice,
    gender: genders.girls,
  },
  {
    num: 27,
    time: "4:30 PM",
    divisions: [divisions.two],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 28,
    time: "4:42 PM",
    divisions: [divisions.one],
    level: levels.juniorVarsity,
    gender: genders.girls,
  },
  {
    num: 29,
    time: "4:54 PM",
    divisions: [divisions.two],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 30,
    time: "5:06 PM",
    divisions: [divisions.one],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 31,
    time: "5:18 PM",
    divisions: [divisions.two],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 32,
    time: "5:30 PM",
    divisions: [divisions.one],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 33,
    time: "5:42 PM",
    divisions: [divisions.two],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 34,
    time: "5:54 PM",
    divisions: [divisions.one],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 35,
    time: "6:06 PM",
    divisions: [divisions.two],
    level: levels.juniorVarsity,
    gender: genders.girls,
  },
  {
    num: 36,
    time: "6:18 PM",
    divisions: [divisions.one],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 37,
    time: "6:30 PM",
    divisions: [divisions.two],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 38,
    time: "6:42 PM",
    divisions: [divisions.one],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 39,
    time: "6:54 PM",
    divisions: [divisions.two],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 40,
    time: "7:06 PM",
    divisions: [divisions.one],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 41,
    time: "7:18 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 42,
    time: "7:30 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 43,
    time: "7:42 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 44,
    time: "7:54 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 45,
    time: "8:06 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 46,
    time: "8:18 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 47,
    time: "8:30 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 48,
    time: "8:42 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 49,
    time: "8:54 PM",
    level: levels.rated,
    gender: genders.girls,
  },
  {
    num: 50,
    time: "9:14 PM",
    level: levels.rated,
    gender: genders.boys,
  },
  {
    num: 51,
    time: "9:34 PM",
    level: levels.sweepstakes,
    gender: genders.girls,
  },
  {
    num: 52,
    time: "9:54 PM",
    level: levels.sweepstakes,
    gender: genders.boys,
  },
];

export const races2023to2025: RaceType[] = [
  {
    num: 1,
    time: "5:00 PM",
    divisions: [divisions.three, divisions.four],
    level: levels.novice,
    gender: genders.boys,
  },
  {
    num: 2,
    time: "5:20 PM",
    divisions: [divisions.three, divisions.four],
    level: levels.novice,
    gender: genders.girls,
  },
  {
    num: 3,
    time: "5:40 PM",
    divisions: [divisions.four],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 4,
    time: "5:52 PM",
    divisions: [divisions.three],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 5,
    time: "6:04 PM",
    divisions: [divisions.four],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 6,
    time: "6:16 PM",
    divisions: [divisions.three],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 7,
    time: "6:28 PM",
    divisions: [divisions.four],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 8,
    time: "6:40 PM",
    divisions: [divisions.three],
    level: levels.juniorSenior,
    gender: genders.girls,
  },
  {
    num: 9,
    time: "6:52 PM",
    divisions: [divisions.four],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 10,
    time: "7:04 PM",
    divisions: [divisions.three],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 11,
    time: "7:16 PM",
    divisions: [divisions.four],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 12,
    time: "7:28 PM",
    divisions: [divisions.three],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 13,
    time: "7:40 PM",
    divisions: [divisions.four],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 14,
    time: "7:52 PM",
    divisions: [divisions.three],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 15,
    time: "8:04 PM",
    divisions: [divisions.four],
    level: levels.juniorSenior,
    gender: genders.girls,
  },
  {
    num: 16,
    time: "8:16 PM",
    divisions: [divisions.three],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 17,
    time: "8:28 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 18,
    time: "8:40 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 19,
    time: "8:52 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 20,
    time: "9:04 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 21,
    time: "9:16 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 22,
    time: "9:28 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 23,
    time: "9:40 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 24,
    time: "9:52 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 25,
    time: "3:50 PM",
    divisions: [divisions.one, divisions.two],
    level: levels.novice,
    gender: genders.boys,
  },
  {
    num: 26,
    time: "4:02 PM",
    divisions: [divisions.one, divisions.two],
    level: levels.novice,
    gender: genders.girls,
  },
  {
    num: 27,
    time: "4:22 PM",
    divisions: [divisions.two],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 28,
    time: "4:34 PM",
    divisions: [divisions.one],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 29,
    time: "4:46 PM",
    divisions: [divisions.two],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 30,
    time: "4:58 PM",
    divisions: [divisions.one],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 31,
    time: "5:10 PM",
    divisions: [divisions.two],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 32,
    time: "5:22 PM",
    divisions: [divisions.one],
    level: levels.juniorSenior,
    gender: genders.girls,
  },
  {
    num: 33,
    time: "5:34 PM",
    divisions: [divisions.two],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 34,
    time: "5:46 PM",
    divisions: [divisions.one],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 35,
    time: "5:58 PM",
    divisions: [divisions.two],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 36,
    time: "6:10 PM",
    divisions: [divisions.one],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 37,
    time: "6:22 PM",
    divisions: [divisions.two],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 38,
    time: "6:34 PM",
    divisions: [divisions.one],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 39,
    time: "6:46 PM",
    divisions: [divisions.two],
    level: levels.juniorSenior,
    gender: genders.girls,
  },
  {
    num: 40,
    time: "6:58 PM",
    divisions: [divisions.one],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 41,
    time: "7:10 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 42,
    time: "7:22 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 43,
    time: "7:34 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 44,
    time: "7:46 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 45,
    time: "7:58 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 46,
    time: "8:10 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 47,
    time: "8:22 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 48,
    time: "8:34 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 49,
    time: "8:54 PM",
    level: levels.rated,
    gender: genders.girls,
  },
  {
    num: 50,
    time: "9:14 PM",
    level: levels.rated,
    gender: genders.boys,
  },
  {
    num: 51,
    time: "9:34 PM",
    level: levels.sweepstakes,
    gender: genders.girls,
  },
  {
    num: 52,
    time: "9:54 PM",
    level: levels.sweepstakes,
    gender: genders.boys,
  },
];

export const races2026toCurrent: RaceType[] = [
  {
    num: 1,
    time: "5:00 PM",
    divisions: [divisions.three, divisions.four],
    level: levels.novice,
    gender: genders.boys,
  },
  {
    num: 2,
    time: "5:20 PM",
    divisions: [divisions.three, divisions.four],
    level: levels.novice,
    gender: genders.girls,
  },
  {
    num: 3,
    time: "5:40 PM",
    divisions: [divisions.four],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 4,
    time: "5:52 PM",
    divisions: [divisions.three],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 5,
    time: "6:04 PM",
    divisions: [divisions.four],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 6,
    time: "6:16 PM",
    divisions: [divisions.three],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 7,
    time: "6:28 PM",
    divisions: [divisions.four],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 8,
    time: "6:40 PM",
    divisions: [divisions.three],
    level: levels.juniorSenior,
    gender: genders.girls,
  },
  {
    num: 9,
    time: "6:52 PM",
    divisions: [divisions.four],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 10,
    time: "7:04 PM",
    divisions: [divisions.three],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 11,
    time: "7:16 PM",
    divisions: [divisions.four],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 12,
    time: "7:28 PM",
    divisions: [divisions.three],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 13,
    time: "7:40 PM",
    divisions: [divisions.four],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 14,
    time: "7:52 PM",
    divisions: [divisions.three],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 15,
    time: "8:04 PM",
    divisions: [divisions.four],
    level: levels.juniorSenior,
    gender: genders.girls,
  },
  {
    num: 16,
    time: "8:16 PM",
    divisions: [divisions.three],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 17,
    time: "8:28 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 18,
    time: "8:40 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 19,
    time: "8:52 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 20,
    time: "9:04 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 21,
    time: "9:16 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 22,
    time: "9:28 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 23,
    time: "9:40 PM",
    divisions: [divisions.four],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 24,
    time: "9:52 PM",
    divisions: [divisions.three],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 25,
    time: "7:30 AM",
    divisions: [divisions.five],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 26,
    time: "7:50 AM",
    divisions: [divisions.five],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 27,
    time: "8:10 AM",
    divisions: [divisions.five],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 28,
    time: "8:30 AM",
    divisions: [divisions.five],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 29,
    time: "8:50 AM",
    divisions: [divisions.five],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 30,
    time: "9:10 AM",
    divisions: [divisions.five],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 31,
    time: "9:30 AM",
    divisions: [divisions.five],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 32,
    time: "9:50 AM",
    divisions: [divisions.five],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 33,
    time: "10:10 AM",
    divisions: [divisions.five],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 34,
    time: "10:30 AM",
    divisions: [divisions.five],
    level: levels.juniorSenior,
    gender: genders.girls,
  },
  {
    num: 35,
    time: "10:50 AM",
    divisions: [divisions.five],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 36,
    time: "3:50 PM",
    divisions: [divisions.one, divisions.two],
    level: levels.novice,
    gender: genders.boys,
  },
  {
    num: 37,
    time: "4:02 PM",
    divisions: [divisions.one, divisions.two],
    level: levels.novice,
    gender: genders.girls,
  },
  {
    num: 38,
    time: "4:22 PM",
    divisions: [divisions.two],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 39,
    time: "4:34 PM",
    divisions: [divisions.one],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 40,
    time: "4:46 PM",
    divisions: [divisions.two],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 41,
    time: "4:58 PM",
    divisions: [divisions.one],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 42,
    time: "5:10 PM",
    divisions: [divisions.two],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 43,
    time: "5:22 PM",
    divisions: [divisions.one],
    level: levels.juniorSenior,
    gender: genders.girls,
  },
  {
    num: 44,
    time: "5:34 PM",
    divisions: [divisions.two],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 45,
    time: "5:46 PM",
    divisions: [divisions.one],
    level: levels.frosh,
    gender: genders.boys,
  },
  {
    num: 46,
    time: "5:58 PM",
    divisions: [divisions.two],
    level: levels.frosh,
    gender: genders.girls,
  },
  {
    num: 47,
    time: "6:10 PM",
    divisions: [divisions.one],
    level: levels.soph,
    gender: genders.boys,
  },
  {
    num: 48,
    time: "6:22 PM",
    divisions: [divisions.two],
    level: levels.soph,
    gender: genders.girls,
  },
  {
    num: 49,
    time: "6:34 PM",
    divisions: [divisions.one],
    level: levels.junior,
    gender: genders.boys,
  },
  {
    num: 50,
    time: "6:46 PM",
    divisions: [divisions.two],
    level: levels.juniorSenior,
    gender: genders.girls,
  },
  {
    num: 51,
    time: "6:58 PM",
    divisions: [divisions.one],
    level: levels.senior,
    gender: genders.boys,
  },
  {
    num: 52,
    time: "7:10 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 53,
    time: "7:22 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 54,
    time: "7:34 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 55,
    time: "7:46 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 56,
    time: "7:58 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.boys,
  },
  {
    num: 57,
    time: "8:10 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.a,
    gender: genders.girls,
  },
  {
    num: 58,
    time: "8:22 PM",
    divisions: [divisions.two],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.boys,
  },
  {
    num: 59,
    time: "8:34 PM",
    divisions: [divisions.one],
    level: levels.varsity,
    heat: heats.b,
    gender: genders.girls,
  },
  {
    num: 60,
    time: "8:54 PM",
    level: levels.rated,
    gender: genders.girls,
  },
  {
    num: 61,
    time: "9:14 PM",
    level: levels.rated,
    gender: genders.boys,
  },
  {
    num: 62,
    time: "9:34 PM",
    level: levels.sweepstakes,
    gender: genders.girls,
  },
  {
    num: 63,
    time: "9:54 PM",
    level: levels.sweepstakes,
    gender: genders.boys,
  },
];

function getRacesForYear(year: number) {
  return year <= 2012
    ? races2007to2012
    : year <= 2022
      ? races2013to2022
      : year <= 2025
        ? races2023to2025
        : races2026toCurrent;
}

export function getNonFeaturedRacesForResults(
  division: DivisionType,
  gender: string,
  year: number,
) {
  const races = getRacesForYear(year);
  return races.filter((race) => race.gender === gender && race.divisions?.includes(division));
}

export function getFeaturedRacesForResults(gender: string, year: number) {
  const races = getRacesForYear(year);
  return races.filter(
    (race) =>
      race.gender === gender && (race.level === levels.rated || race.level === levels.sweepstakes),
  );
}

function getRacesByNumber(min: number, max: number): RaceType[] {
  const races = getRacesForYear(new Date().getFullYear());
  return races.filter((race) => race.num >= min && race.num <= max);
}

export const filteredRaces = {
  fridayRaces: getRacesByNumber(1, 24),
  saturdayMorningRaces: getRacesByNumber(25, 35),
  saturdayNightNonFeaturedRaces: getRacesByNumber(36, 59),
  saturdayNightFeaturedRaces: getRacesByNumber(60, 63),
};
